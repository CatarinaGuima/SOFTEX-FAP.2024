...
const areaRtangulo = (baseTriangulo * alturaTriangulo) / 2;
console.log(`A área do triângulo = ${areaRtangulo.toFixed(2)}`);
...
const baseRetangulo = Number(prompt("Informe a base do Retângulo: "));
...
let somaNum = 0;
for (let i = 1 ; i <= 3 ; i++) {
    const numero = parseFloat(prompt(`Digite o ${i}º número decimal: `)!);
    somaNum += numero;
}
const media = somaNum / 3;
console.log(`A média Aritmétrica dos números: ${media.toFixed(2)}`);
...
const idadeAgora = parseInt(prompt("Digite a sua idade atual: ")!);
const idadeMeses = idadeAgora * 12;
const idadeDias = idadeAgora * 365;
console.log(`Você já viveu ${idadeMeses} meses e ${idadeDias} dias.`);
...
const valorEmReais = parseFloat(prompt("Informe o valor em reais R$: ")!);
const cotacaoDolar = parseFloat(prompt("Informe a cotação do dólar: ")!);
const valorEmDolar = valorEmReais / cotacaoDolar;
console.log(`O valor convertido é US$: ${valorEmDolar.toFixed(2)} dólares`);

//Questão 41
// Solicite ao usuário para digitar um número decimal e arredonde-o para o inteiro mais próximo.
const numberDec = parseFloat(prompt("Escolha um número decimal: ")!);
const numberInt = Math.round(numberDec);
...
let somaN = 0;
for (let i = 1 ; i <= 2; i++){
    const n = parseInt(prompt(`Digite o ${i}º número inteiro: `)!);
    somaN += n;
}
const n3 = parseInt(prompt(`Digite o 3º número inteiro: `)!);
const resposta = (somaN) * n3;
